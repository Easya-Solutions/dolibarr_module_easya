...
class ActionsEasya
{
    ...
    function printLeftBlock($parameters, &$object, &$action, $hookmanager)
    {
        ...
        if (!empty($maintenance_file_path) && (file_exists($maintenance_file_path) || file_exists(dol_buildpath(preg_replace('/^\/htdocs/', '', $maintenance_file_path))))) {
            ...
            $this->resprints.= '    $(() => {
                $(".side-nav-vert").before(\'<div class="warning" style="background-color: red; color: white !important; font-weight: bold; font-size: 20px; '.$add_style.'">MODE MAINTENANCE</div>\') 
